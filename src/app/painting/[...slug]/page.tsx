...
import { menuForDesktopItems, sortOptions } from "@/utils/commonJson";
...
const getPaintingRoutes = () => {
  const paintingMenu = menuForDesktopItems.find(
    (menu) => menu.title === "Painting"
  );
  if (!paintingMenu || !paintingMenu.childMenus) return [];
  return paintingMenu.childMenus.map((child) =>
    child.navigation.replace("/painting/", "")
  );
};

const getPaintingPageInfo = (category: string) => {
  const paintingMenu = menuForDesktopItems.find(
    (menu) => menu.title === "Painting"
  );
  if (!paintingMenu || !paintingMenu.childMenus) return null;

  return paintingMenu.childMenus.find((child) =>
    child.navigation.endsWith(category)
  );
};
...
const PaintingTypesPage = ({ params }: Props) => {
  ...
  const pageInfo = getPaintingPageInfo(category);

  if (!getPaintingRoutes().includes(category)) {
    return notFound();
  }
